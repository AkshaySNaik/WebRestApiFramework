...
public class TC003_PUTdetailsTest extends BaseClass {
	...
	public void PUTDetailsTest(String datasupply) {

		log.info("********** Started TC003_PutDetailsTest **************");

		String[] putdata = datasupply.split(",");
		
		RestAssured.baseURI = URI;
		httprequest = RestAssured.given();

		JSONObject data = new JSONObject();
		data.put("id", putdata[0]);
		data.put("username", putdata[1]);
		data.put("firstName", putdata[2]);
		data.put("email", putdata[3]);
		data.put("password", putdata[4]);
		data.put("phone", putdata[5]);

		httprequest.header("Content-Type", "application/json");
		httprequest.body(data.toJSONString());

		response = httprequest.request(Method.PUT, "v2/user/" + putdata[1]);
		try {
			Thread.sleep(5000);
		} catch (Exception e) {
			e.printStackTrace();
		...
		Assert.assertTrue(responsebody.contains(putdata[0]));
