...
public class TC006_GETpendTest extends BaseClass {

	@Test(priority = 1)
	public void GETPendingTest() {

		log.info("********** Started TC006_GetPendingDetails **************");

		RestAssured.baseURI = URI;
		httprequest = RestAssured.given();
		response = httprequest.request(Method.GET, "v2/pet/findByStatus?status=pending");
	...
	public void checkResponseBody() {
		...
		Assert.assertTrue(responsebody.contains("pending"));
		Assert.assertTrue(responsebody.contains("non velit anim sint"));
		Assert.assertTrue(responsebody.contains("12345"));
	}
